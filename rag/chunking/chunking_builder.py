...
# Sử dụng logger của Airflow để chắc chắn log hiển thị trong UI task logs
...
def chunk_documents(
    ...
    **kwargs,
    ...
    if isinstance(documents, str):
        ...
        logger.info(f"[chunk_documents] Deserialized documents from XCom")
    
    text_splitter = get_text_splitter(type)
    # Lọc kwargs dựa trên signature của hàm splitter (vd: create_size_based_character_splitter)
    # thay vì __init__ của object function, nếu không chunk_size, chunk_overlap sẽ bị drop.
    kwargs = filter_kwargs(text_splitter, kwargs)
    ...
    logger.info(f"[chunk_documents] Serialized {len(chunks)} chunks for XCom")
